...
const RegisterClient = () => {
    ...
    const [remedioInput, setRemedioInput] = React.useState([{ remedio: '', horario: [], doses: 0, foto: "",valor:'' }]);
...
`;
    ...
    async function setNewClient() {
        ...
        const uploadPromises = remedioInput.map(async (response, remedioIndex) => {
            ...
            set(ref(database, databasePath), {
                ...
                valorMedicamento:response.valor
            }).then(() => {
            ...
            const somaValores = remedioInput.reduce((total, remedio) => {
                return total + parseFloat(remedio.valor); // Garante que seja um número
            }, 0);
        
            await update(ref(database, `${base64.encode(user.email)}/relatorios/clientes`), {
                clientes: Number(relatorio.clientes) + 1, // Incrementa o número de clientes
                valorTotalM: Number(relatorio.valorTotalM) + Number(somaValores) // Atualiza o valor total com a soma dos valores de remedioInput
            })
            .then(() => {
    ...
    const handleInputChangeValor = (remedioInput1, newValue) => {
